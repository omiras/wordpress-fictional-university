...
class Search {
  ...
  getResults() {
    ...
    $.getJSON(universityData.root_url + '/wp-json/wp/v2/pages?search=' + this.searchField.val())    
        ...
        // The first item is the actual JSON itself.
        // In others array's positions we have information about the request
        ...
        ${combineResults.length ? '<ul class="link-list min-list">' : '<p>No general information matches that search.</p>'}
          ${combineResults.map(item => `<li><a href="${item.link}">${item.title.rendered}</a> by ${ item.type == 'post' ? `by ${item.authorName}` : ''} </li>`).join('')}
